...
# 配置方向: '1'(横置き)だとカードを縦に持ったとき文字が90度回転してしまう(実機確認)。
# 宝飾ナビの券面と同じ「縦持ちで横書き」にするため '0'(縦置き+上書き)を使う。
# 縦置きの座標系は X0〜319(幅)・Y0〜479(長さ)。白枠の正確な範囲は縦置きグリッドで測り直す。
FACE_DIR = "0"
FACE_LABEL_X = 20     # 項目名のX(縦置きグリッドの結果で調整)
FACE_VALUE_X = 130    # 値のX
FACE_MAX_X = 300      # 白枠の右端(暫定。グリッドで確定)
FACE_TOP_Y = 60       # 1行目のY(Y小=上と仮定。グリッドで確認)
FACE_LINE_H = 50      # 行送り(ドット)
