...
def _digits(s):
    """数字だけを取り出す。★全角の数字(０９０…)も拾う(2026-09-02)。
    以前は半角だけを見ていたため、全角で入っている電話番号が「数字なし」になり、
    重複チェックの対象から外れていた。宝飾ナビから引き継いだデータには
    全角で入力された番号が混じり得るので、幅をそろえてから取り出す。"""
    return re.sub(r"[^0-9]", "", unicodedata.normalize("NFKC", str(s or "")))
...
TEL_MIN_DIGITS = 6   # これ未満は電話番号とみなさない(内線・入力途中を拾わないため)


def _tel_key(t):
    """電話番号を比較用に正規化する。短すぎる番号と「ダミー番号」は None(判定に使わない)。

    実データには 1111111111 / 1111-11-1111 のような穴埋めの番号が大量にあり、
    これを「同じ電話」と数えると無関係な顧客同士が重複扱いになる(2026-08-13 テストで発覚)。
    数字の種類が2種類以下の番号はダミーとみなす。

    ★2026-09-02 修正: 下限を9桁から**6桁**に下げた。店では市外局番を書かない
    「32-0688」のような書き方が普通で、数字にすると6桁しかない。9桁の条件だと
    **その顧客は全員「電話なし」扱いになり、重複チェックが一度も働かなかった**
    (実際に同じ人が二重登録された)。ダミーの判定は「数字の種類が2種類以下」で
    足りるので、桁数で弾く必要はない(000000・111111 はそちらで除外される)。
    ※この関数を使う判定は**いずれも「氏名と電話の両方が一致」**を条件にしているため、
      桁数を下げても無関係な人が重複扱いになることはない。
    """
    d = _digits(t)
    if len(d) < TEL_MIN_DIGITS or len(set(d)) <= 2:
        return None
    return d
