...
def main():
    ...
    for r in rows("d_hanbai"):
        ...
        # ★「0」は伝票番号ではない(2026-09-16 修正)。宝飾ナビは2011年から伝票番号
        #   (YYMMDD+連番)を振り始めており、**それ以前の売上はすべて 0** が入っている。
        #   以前はこれを番号として扱っていたため、**全顧客の2011年より前の売上が
        #   「伝票0番」という1枚の伝票にまとまり、最初の1人にだけ紐づいて、
        #   他の全員の古い購入履歴が消えていた**(買上日もその1枚の日付に潰れていた)。
        #   累計購入額・顧客ランク・DM抽出まで狂うので、番号なしとして1行ずつ独立させる。
        if dp in ("0", "0.0", "-0"):
            dp = ""
